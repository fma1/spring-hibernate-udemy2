...
//  def getById[T: Manifest](id: java.lang.Long) : T = session.get(manifest[T].runtimeClass, id).asInstanceOf[T]
   ...
   */
  def find[T](hql: String, params: Any*): util.List[T] = {
    createQuery(hql, params: _*).getResultList.asInstanceOf[util.List[T]]
  }
  ...
  private def createQuery(queryString: String, values: Any*) = {
    require(queryString != null && queryString.nonEmpty, "queryString cannot be empty")
...
object SessionWrapper {
  ...
  implicit def sessionToSessionWrapper(s: Session) : SessionWrapper = SessionWrapper(s)
