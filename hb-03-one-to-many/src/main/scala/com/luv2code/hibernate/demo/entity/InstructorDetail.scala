...
class InstructorDetail(_youtubeChannel: String, _hobby: String) {
  ...
  @BeanProperty
  // Instructor owns the relationship
  @OneToOne(mappedBy = "instructorDetail", cascade = Array(CascadeType.PERSIST, CascadeType.MERGE, CascadeType.DETACH, CascadeType.REFRESH))
